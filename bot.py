# V30 MASTER ULTIMATE BATCH VIP — FIXED SIGNAL ENGINE
# Only signal discovery and emission logic has been enhanced.
...
def verify_blockchain_transaction(tx_signature, expected_currency="USDC"):
    ...
    try:
        ...
        # USDC is an SPL token with 6 decimals.
        received_units = 0
...
def _elite_get_market_tokens():
    """Retrieve market tokens with fallback to direct fetch if cache is empty."""
    global _elite_market_cache, _elite_market_cache_time, _elite_market_refresh_thread
    ...
    # Try to read cache
    with state_lock:
        cache = list(_elite_market_cache)
        cache_time = _elite_market_cache_time

    now = time.time()
    # If cache is empty or older than allowed, fetch directly (fallback)
    if not cache or (now - cache_time) > ELITE_DISCOVERY_MAX_AGE_SECONDS:
        logger.info("🔄 کش توکن‌ها خالی یا قدیمی است؛ دریافت مستقیم از DexScreener...")
        try:
            direct = get_real_market_trending_tokens()
            if direct:
                # Update cache with direct results
                with state_lock:
                    _elite_market_cache = list(dict.fromkeys(direct))[:ELITE_MAX_UNIQUE_TOKENS]
                    _elite_market_cache_time = time.time()
                return list(_elite_market_cache)
            else:
                # If direct fetch also fails, return a set of well-known tokens as last resort
                fallback_tokens = [
                    ...
                    "BQCDHoiB3HyK2ByRpyjP9DqXr7hE9UpBgBrYtP6pj2LB",  # BONK
                    "mSoLzYCxHdYgdzU16g5QSh3i5K3z3KZK7ytfqcJm7So",   # mSOL
                    "DezXAZ8z7PnrnRJjz3wXBoRgixCa6xjnB7YaB1pPB263"    # SAMO
                ]
                logger.warning("⚠️ هیچ توکنی از DexScreener دریافت نشد؛ از لیست پشتیبان استفاده می‌شود.")
                return fallback_tokens
        except Exception as e:
            logger.error(f"❌ Fallback fetch failed: {e}")
            # Return whatever we have in cache (maybe empty) but at least we tried
            return cache
...
def unified_market_scanner_loop(app):
    ...
    while True:
        ...
        try:
            ...
            # اگر توکن‌ها خالی بود، یک بار دیگر تلاش مستقیم (fallback قبلاً در _elite_get_market_tokens انجام شده)
            if not tokens:
                logger.warning("⚠️ هیچ توکنی برای اسکن وجود ندارد؛ ۱ ثانیه صبر می‌کنیم...")
                time.sleep(1)
                continue
